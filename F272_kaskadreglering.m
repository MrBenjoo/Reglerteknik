...
for k=1:N % the loop will run N times, each time takes exactly dT seconds
    ...
    if(mod(k-1, R) == 0)
        ...
        y1(k) = a.analogRead('a1'); %läser av undre vattentanken
        if(k>1)
            e1(k) = r(k)-y1(k);
            w1(k) = w1(k-1) + e1(k);
        ...
        signalYttreSystem = u1(k);
    else
        % in till R2 inre kretsen, ?vre vattentanken
        y2(k) = a.analogRead('a0'); %läser av ovre vattentanken
        if(k>1)
             disp('signalYttreSystem')
                signalYttreSystem
            e2(k) = signalYttreSystem -y2(k);
            w2(k) = w2(k-1)+e2(k);
